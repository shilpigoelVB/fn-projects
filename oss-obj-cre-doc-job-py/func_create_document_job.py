...
def analyze_document_bulk(config, signer, namespace, bucket_name, object_name, output_bucket, prefix):
    ...
    search_object=prefix+"/"+resp.data.id+"/"+namespace+"_"+object_name+".json"
    logging.getLogger().debug(" Search object Name is {0} "+search_object_name)
    ...
    try:
        ...
        object = client.get_object(namespace, output_bucket, search_object_name)
